...
fn split_block(block: &Block,
               ...
               min_split_size: usize) -> Option<(Vote, Vote)>
{
       ...
       neighbours_ok(block, current_blocks, min_split_size)
    {
        ...
        let v0 = Vote { from: block.clone(), to: b0 };
        let v1 = Vote { from: block.clone(), to: b1 };
...
fn neighbours_ok(block: &Block, current_blocks: &CurrentBlocks, min_split_size: usize) -> bool {
    current_blocks.iter()
        .filter(move |other_block| other_block.prefix.is_neighbour(&block.prefix) ||
